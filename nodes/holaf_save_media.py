...
class HolafSaveMedia:
    ...
    RETURN_NAMES = ("filepath", "prompt", "workflow_json",)
    FUNCTION = "save_media"
    OUTPUT_NODE = True
    CATEGORY = "Holaf"
    ...
    def save_media(self, mode, **kwargs):
        ...
        if mode == "image":
            if image_tensor is None:
                ...
                return {"ui": {"text": ["No image provided"]}, "result": ("", "", "")}
            ...
            return {"ui": {"images": results}, "result": (final_path, prompt, workflow_json)}
            ...
            return {"ui": {ui_key: results}, "result": (video_path, prompt, workflow_json)}


        elif mode == "audio":
            if audio_data is None:
                print("[Holaf Save Media] Warning: Mode is 'audio' but no audio provided.")
                return {"ui": {"text": ["No audio provided"]}, "result": ("", "", "")}
            ...
            return {"ui": {"audios": results}, "result": (audio_path, prompt, workflow_json)}
