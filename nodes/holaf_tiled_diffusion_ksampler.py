...
class AbstractDiffusion:
    def __init__(self, tile_batch_size: int = 1):
        self.w, self.h = 0, 0
    ...
    def get_tile_weights(self) -> Union[torch.Tensor, float]: return 1.0
...
class MultiDiffusion(AbstractDiffusion):
    ...
    def __call__(self, model_function, args):
        ...
        num_batches = math.ceil(len(self.all_bboxes) / self.tile_batch_size)
        for i in range(num_batches):
            start_idx = i * self.tile_batch_size
            end_idx = min((i + 1) * self.tile_batch_size, len(self.all_bboxes))
            current_bboxes = self.all_bboxes[start_idx:end_idx]

            if not current_bboxes: continue

            x_tiles = torch.cat([x_in[bbox.slicer] for bbox in current_bboxes], dim=0)
            t_tiles = repeat_to_batch_size(t_in, x_tiles.shape[0])
            c_tiles = {k: repeat_to_batch_size(v, x_tiles.shape[0]) if isinstance(v, torch.Tensor) else v for k, v in c_in.items()}
            
            x_tile_out = model_function(x_tiles, t_tiles, **c_tiles)

            for j, bbox in enumerate(current_bboxes):
                self.x_buffer[bbox.slicer] += x_tile_out[j*N:(j+1)*N]
...
class MixtureOfDiffusers(AbstractDiffusion):
    def __init__(self, tile_batch_size: int = 1):
        super().__init__(tile_batch_size)
    ...
    def __call__(self, model_function, args):
        ...
        for i in range(num_batches):
            ...
            for j, bbox in enumerate(current_bboxes):
                ...
                self.x_buffer[bbox.slicer] += x_tile_out[j*N:(j+1)*N] * w
...
class SpotDiffusion(AbstractDiffusion):
    ...
    def __call__(self, model_function, args):
        ...
        if self.uniform_distribution is None and self.sigmas is not None:
            gen = torch.Generator(device='cpu').manual_seed(self.seed)
            shift_height = torch.randint(0, self.tile_height, (len(self.sigmas)-1,), generator=gen, device='cpu')
            shift_width = torch.randint(0, self.tile_width, (len(self.sigmas)-1,), generator=gen, device='cpu')
            self.uniform_distribution = (shift_height, shift_width)
        
        sh_h, sh_w = 0, 0
        if self.uniform_distribution and self.sigmas is not None:
            cur_i = find_nearest_in_list(t_in[0], self.sigmas)
            if cur_i < len(self.uniform_distribution[0]):
        ...
        x_in_rolled = x_in
        if (sh_h, sh_w) != (0,0):
            x_in_rolled = torch.roll(x_in, shifts=(sh_h, sh_w), dims=(-2,-1))
        ...
        for i in range(num_batches):
            ...
            x_tiles = torch.cat([x_in_rolled[bbox.slicer] for bbox in current_bboxes], dim=0)
            t_tiles = repeat_to_batch_size(t_in, x_tiles.shape[0])
            c_tiles = {k: repeat_to_batch_size(v, x_tiles.shape[0]) if isinstance(v, torch.Tensor) else v for k, v in c_in.items()}
            x_tile_out = model_function(x_tiles, t_tiles, **c_tiles)
...
class HolafTiledDiffusionKSampler:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "max_tile_size": ("INT", {"default": 1024, "min": 256, "max": 8192, "step": 8}),
    ...
    def calculate_tile_params(self, pixel_w, pixel_h, max_tile_size, overlap_size):
        ...
        step_w = tile_w_init - overlap_size if tile_w_init > overlap_size else tile_w_init
        step_h = tile_h_init - overlap_size if tile_h_init > overlap_size else tile_h_init
        
        x_slices = 1 if tile_w_init >= pixel_w else 1 + math.ceil((pixel_w - tile_w_init) / step_w)
        y_slices = 1 if tile_h_init >= pixel_h else 1 + math.ceil((pixel_h - tile_h_init) / step_h)
    ...
    def sample_advanced_tiled(self, model: ModelPatcher, positive, negative, vae,
                             ...
                             method, max_tile_size, overlap_size, tile_batch_size,
        ...
        impl_class = {"Mixture of Diffusers": MixtureOfDiffusers, "MultiDiffusion": MultiDiffusion, "SpotDiffusion": SpotDiffusion}.get(method)
        if impl_class is None: raise ValueError(f"Unknown Tiling Method: {method}")
        
        impl = impl_class(tile_batch_size)
        print(f"  -> Using method: {method} with tile batch size of {tile_batch_size}.")
        ...
        try:
            impl.compression = model.model.model_config.latent_format.scale_factor
        except AttributeError:
            impl.compression = 8

        # --- CORRECTION FINALE : Conversion en int() ---
        impl.tile_width = int(final_tile_w_px // impl.compression)
        impl.tile_height = int(final_tile_h_px // impl.compression)
        impl.tile_overlap = int(final_overlap_px // impl.compression)

        old_wrapper = None
        if hasattr(model.model, 'model_function_wrapper'):
            old_wrapper = model.model.model_function_wrapper
        
        sampled_latent_samples = None
        try:
            ...
            sampled_latent_samples = comfy.sample.sample(model, noise, steps, cfg, sampler_name, scheduler, 
                                                         positive, negative, latent_samples, denoise=denoise, 
                                                         disable_noise=disable_noise, callback=callback, seed=seed)
        finally:
            print("  -> Restoring original model function.")
            model.set_model_unet_function_wrapper(old_wrapper)
